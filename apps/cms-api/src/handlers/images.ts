...
imagesHandler.post("/", async (c) => {
  ...
  const id = generateId();
  const ext = getExtension(file.name) || getExtensionFromMime(file.type) || "bin";
  ...
  await c.env.DB.prepare(
    `INSERT INTO images (id, article_id, filename, original_filename, r2_key, mime_type, size_bytes, alt_text)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).bind(id, articleId, filename, file.name, r2Key, file.type, file.size, altText).run();
...
imagesHandler.get("/article/:articleId", async (c) => {
  ...
  const images: Image[] = (results || []).map((row) => mapRowToImage(row, c.env));
