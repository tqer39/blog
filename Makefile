...
.PHONY: mise-setup
mise-setup: ## Trust mise config and install tools
	@if command -v mise >/dev/null 2>&1; then \
		mise trust 2>/dev/null || true; \
		echo "Installing mise tools..."; \
		mise install; \
		echo "✓ mise ツールをインストールしました"; \
	else \
		echo "⚠ mise が見つかりません"; \
	fi

.PHONY: bootstrap
bootstrap: install-brew brew-bundle mise-setup ## Install Homebrew and show next steps
	@echo ""
